...
cat << EOS > config_minimal-surface-54-fragment
# config referenced: Arch Linux linux-lts (https://git.archlinux.org/svntogit/packages.git/plain/trunk/config?h=packages/linux-lts&id=d3707b98a2ade5a569905918e903a4efa2140bd3)
...
# After commit https://chromium.googlesource.com/chromiumos/third_party/kernel/+/b9fd280765020857a93d8f45b76f37f020193cc4
# ("FROMGIT: BACKPORT: platform/x86: intel_pmc_ipc: Convert to MFD"),
# and now available as MFD_INTEL_PMC_BXT. But according to that commit,
...
# Considering that CHTCRC_PMIC_OPREGION is not available on 4.19 but
# S0ix can be achieved anyway, so not enabling.
...
CONFIG_DPTF_POWER=y
EOS
cleanup_config_fragment config_minimal-surface-54-fragment
